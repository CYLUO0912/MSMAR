...
library(mvmeta)
mlprof.fn_s <- function (par,Xstar, Ystar, Cmatrix, D, k, m){
...
mlprof.fnU_s <- function (par,Xstar, Ystar, Cmatrix, V, D, k, m){
...
mlprof.fnV_s <- function (par,Xstar, Ystar, U, D, k, m){
  V <- par2V_s(par,k)
  gls <- glsfit_s(Xstar, Ystar, D, V, U, onlycoef = FALSE)
  pconst <- -0.5 * m * k * log(2 * pi)
  pres <- -0.5 * (crossprod(gls$invtMY - gls$invtMX %*% gls$coef))
...
remlprof.fn_s <- function (par,Xstar, Ystar, Cmatrix, D, k, m){
  U <- chol2inv(chol(diag(m) - par[1] * Cmatrix))
  V <- par2V_s(par[-1],k)
  gls <- glsfit_s(Xstar, Ystar, D, V, U, onlycoef = FALSE)
...
remlprof.fnU_s <- function (par,Xstar, Ystar, Cmatrix, V, D, k, m){
  U <- chol2inv(chol(diag(m) - par * Cmatrix))
  gls <- glsfit_s(Xstar, Ystar, D, V, U, onlycoef = FALSE)
  pconst <- -0.5 * (m * k - length(gls$coef))* log(2 * pi)
  pres <- -0.5 * (crossprod(gls$invtMY - gls$invtMX %*% gls$coef))
...
remlprof.fnV_s <- function (par,Xstar, Ystar, U, D, k, m){
  V <- par2V_s(par,k)
  gls <- glsfit_s(Xstar, Ystar, D, V, U, onlycoef = FALSE)
  pconst <- -0.5 * (m * k - length(gls$coef))* log(2 * pi)
  pres <- -0.5 * (crossprod(gls$invtMY - gls$invtMX %*% gls$coef))
...
remlprof.grV_s <- function (par, Xstar, Ystar, U, D, k, m){
  L <- diag(0, k)
...
opt.iter.UV_s <- function(fnU,fnV,V,opt.iter,opt.iter.method,Xstar,Ystar,
...
smvmeta.ml <- function (Xstar, Ystar, D = D, Cmatrix = Cmatrix, k, m, p, bscov, 
  ...
  par <- opt.iter.UV_s(fnU = fnU,fnV = fnV,V = V,opt.iter = control$opt.iter,
